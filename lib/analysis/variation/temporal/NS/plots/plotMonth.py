...
for src in ['S', 'N']:
    ...
    with open(basedir+'NS/SplotData.txt', 'r') as f:
        datas = f.readlines()
        for i in range(len(datas)):
            datas[i] = datas[i][:-1]

    with open(basedir+'NS/SplotTimes.txt', 'r') as f:
        times = f.readlines()
...
plt.title('Detection count trend by month for Southern Hemisphere observers', y=1.05)
