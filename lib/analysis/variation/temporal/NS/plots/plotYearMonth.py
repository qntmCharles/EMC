...
for src in ['S', 'N']:
    ...
    with open(basedir+'NS/'+src+'plotData.txt', 'r') as f:
        datas = f.readlines()
        for i in range(len(datas)):
            datas[i] = datas[i][:-1]

    with open(basedir+'NS/'+src+'plotTimes.txt', 'r') as f:
        times = f.readlines()
    ...
    plt.errorbar(plotTimes, finalData, yerr= finalErr, label=src)

plt.title('Detection count trend by year and month for Southern Hemisphere observers', y=1.05)
