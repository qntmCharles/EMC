...
def baseline(entries, activeRange):
    ...
    b = sum(allData)/len(allData)
    plt.scatter(allTimes, allData)
    plt.plot(allTimes, [b]*len(allTimes))
    plt.show()

    return sum(allData)/len(allData)
    ...
    for observer, observerDates in observers.items():
        ...
        if (longitude is not None) and (latitude is not None):
            ...
            for date in sorted(observerDates):
                ...
                #plt.plot(plotDates, [b for m in range(len(plotDates))])
                #plt.plot(plotDates, zhrs)
                #plt.show()
